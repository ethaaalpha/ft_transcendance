...
class Match(models.Model):
	...
	score[0] = host
	score[1] = invited
	state = 0 created, 1 started, 2 finished
	"""
	def default_ready():
		return ([False, False])
	...
	def wait(match_id, host_username, invited_username):
		...
		time.sleep(20)
		match = Match.getMatch(id=match_id)
	...
	def finish(self, score: tuple, **kwargs):
		...
		self.setState(2)
		self.setData({'distance': kwargs['distance'], 'duration': self.duration.total_seconds(), 'pong': kwargs['tap']})
		setOutMatch(loser)
...
class Room(models.Model):	
	...
	def _runRoom(self):
		if (self.opponents.count() != int(self.mode[0])):
			return
		self.state = 1
		self.save()
